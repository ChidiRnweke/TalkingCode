...
<div class={cn('group relative size-24 overflow-hidden rounded-lg', className)} {...restProps}>
	{#if isImage}
		...
		{#if onRemove}
			<Button
				...
				class="absolute top-2 right-2 size-6 rounded-full bg-background/80 p-0 opacity-0 backdrop-blur-sm transition-opacity group-hover:opacity-100 hover:bg-background [&>svg]:size-3"
	...
	{:else}
		...
		{#if onRemove}
			<Button
				aria-label="Remove attachment"
				class="size-6 shrink-0 rounded-full p-0 opacity-0 transition-opacity group-hover:opacity-100 hover:bg-accent [&>svg]:size-3"
				onclick={handleRemove}
				type="button"
				variant="ghost"
			>
