...
import structlog
from talkingcode.domain.models import ExecuteToolGroupInput, ToolExecutionResult
...
class ToolRegistry:
    ...
    def _validate_value_against_schema(
        ...
        *,
        ...
        if expected_type == "string" and not isinstance(value, str):
            raise ToolContractError(
                "invalid_tool_arguments", f"{path} must be a string"
            )
        ...
        if expected_type == "integer" and (
            ...
            raise ToolContractError(
                "invalid_tool_arguments", f"{path} must be an integer"
            )
        ...
        if expected_type == "number" and (
            ...
            raise ToolContractError(
                "invalid_tool_arguments", f"{path} must be a number"
            )

        if expected_type == "boolean" and not isinstance(value, bool):
            raise ToolContractError(
                "invalid_tool_arguments", f"{path} must be a boolean"
            )
    ...
    async def execute_group(
        ...
        async def execute_single(
            ...
            start = time.time()
            try:
                timeout_seconds = self._timeouts.get(
                    tool_name, input_data.timeout_seconds
                )
